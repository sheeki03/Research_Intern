    ...
    async def _analyze_market_movement(self, direction: str, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if direction == "down":
                ...
                analysis += f"• Bitcoin: ${btc_data.current_price:,.2f} ({btc_change:+.2f}%)\n"
                analysis += f"• Ethereum: ${eth_data.current_price:,.2f} ({eth_change:+.2f}%)\n"
                analysis += f"• Total Market Cap: {market_change:+.2f}% (24h)\n\n"
                ...
                analysis += f"💡 **Insight**: Positive momentum often attracts more buyers, but be cautious of overextension."
